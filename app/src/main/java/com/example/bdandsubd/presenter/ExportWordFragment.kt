...
class ExportWordFragment : Fragment() {
    ...
    fun openDocument(name: File) {
        ...
        val fileUri = FileProvider.getUriForFile(requireContext(), "com.example.bdandsubd.fileprovider", file)
    ...
    fun createDocx(filepath: File) {
        try {
            val xwpfDocument = XWPFDocument()
                ...
                val headers = listOf("ID", "Название отеля", "Имя гостя", "Номер комнаты", "Тип комнаты", "Цена")
